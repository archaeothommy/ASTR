...
- See faulty function `testfun` in `R/test.R`
- Build the package
...
library(testthat)
# working
test_that("testfun behaves as expected for specific input", {
...
Example: https://github.com/ropensci/c14bazAAR (`tests/testthat`)

#### Golden/Snapshot tests

Validate the output of a function against a pre-defined, supposedly correct, "golden" output.

Example: https://github.com/nevrome/currycarbon (`test/golden`)
...
- :warning: The exact rendering of a plot depends on many factors. Plot tests tend to be brittle.
...
```{r}
testfun2 <- function(x) { log(x) }
# working
test_that("testfun2 behaves as expected for specific input", {
  expect_lt(testfun2(3), 3)
})
# but
test_that("testfun2 behaves as expected for specific input", {
  expect_lt(testfun2(-1), -1)
})
```

- The quickcheck R package (named after its Haskell ancestor) allows to test more generally for properties of functions, by trying lots of random inputs.
- https://github.com/armcn/quickcheck

```{r}
library(quickcheck) # add to tests/testthat/setup.R
test_that("testfun2 won't fail with any numerical input", {
  for_all(
    a = numeric_(len = 1),
    property = function(a) { expect_lt(testfun2(a), a) },
    tests = 100
  )
})
```

:warning: Running many of iterations frequently may be wasteful.

## Exercise

- Clone the ASTR repo: `git clone git@github.com:archaeothommy/ASTR.git`
- Switch to this branch: `git switch debugging_testing_demonstration`
- Place the following function in a new .R file `reverse.R`
...
It takes a vector and reverses its elements: `reverse(c(1,2,3,4))`.

- Build the package
- Add meaningful tests in a new file `tests/testthat/test_reverse.R` and run them
