...
This vignette outlines how the conventions defined in [the ASTR
schema](https://archaeothommy.github.io/ASTR/articles/VG.ASTRschema.0.0.2.md)
were implemented in the ASTR package.
...
- Units will be removed from column names because {units} stores them in
  ...
  with `recover_unit_names = TRUE`.

## Units

- The package relies on [units](https://r-quantities.github.io/units/),
  which uses the
  [udunits](https://www.unidata.ucar.edu/software/udunits) C library,
  for handling all SI units (e.g. *µg/ml*) and relative concentration
  units (e.g. *ppm(m/V)*). If the mixture type is not specified, *m/m*
  is assumed. Non-SI units *wt%* and *at%* were defined as `wtP` and
  `atP`, respectively.
...
- The unit *wt%* (weight%) is defined as relative unit `wtP` analogous
  ...
  columns are summarized into a single column per element.  
- Conversion to and from *at%* (`atP`) is restricted to *wt%* (`wtP`).
  If you want to convert to or from *at%* in another unit, you must
  convert to *wt%* first.
