...
To make the most out of the features offered by the ASTR package, the
object should be imported as ASTR object. Because they adhere to the
conventions set out in the [ASTR
schema](https://archaeothommy.github.io/ASTR/articles/VG.ASTRschema.0.0.2.md),
a lot of information can be extracted and used for efficient and
convenient handling of archaeometric and geochemical data. This is also
why many of the package functions require only the data set as input,
making it convenient for persons less experienced in R. This article
highlights some of these functionalities.
...
imports spreadsheets in a variety of formats, including Excel files, and
passes then on to the function
[`as_ASTR()`](https://archaeothommy.github.io/ASTR/reference/ASTR.md).
For this article, I created some mock-up data:
...
which do already have the column headers organised according to the ASTR
conventions. We also see that there were some problems with the copper
isotope data: it seems that some Excel formulas did not work as
intended. To convert the data to ASTR, we have to define one of the
columns as the column with the IDs of the data and and any column
providing contextual information, i.e. no analytical values (cf. [ASTR
schema:
...
#> Warning in as_ASTR(data, id_column = "Group", context = "Group"): Detected
...
#> analytical columns
#> Warning in as_ASTR(data, id_column = "Group", context = "Group"): See the full
#> list of validation output with: ASTR::validate(<your ASTR object>).
```

While ASTR attempts to make data import as easy as possible, it also
aims to be as transparent about it as possible. Consequently, the
function issued several warnings, flagging (potential) issues with our
data set:
...
- Some values in the column `d65Cu` were replaced with `NA`, when
  inspecting the data set, it is clear that these were the Excel error
  messages. In general, ASTR replaces by default common and user-defined
  indicators for missing values and also for indicators of “below
  detection limit” during import.
- Across the dataset, 34 values are missing.

To learn more about the (potential) issues with out dataset, it suggests
to run
...
This summary of where to find the missing values comes in very handy for
(very) large datasets!
...
wrong with the Excel file. In case of the lead isotop data, however,
they are intentional (they were not measured for these samples).
...
have a measurement unit R can understand (note that the unit `wt%` must
be written `wtP`)[¹](#fn1):
...
case, we can take advantage that these are the only ones values in
`wtP`:
...
While continuing investigating the metal, we want to estimate its
...
first. That’s why we have to run
[`unify_concentration_unit()`](https://archaeothommy.github.io/ASTR/reference/ASTR.md)
first: is detects values with units that can be converted into the
...
type and lead isotope model age parameters to our hard drive. For this
task, ASTR relies on the functions already provided by R and other R
packages. Before export, we have to make sure that the information about
the units of the values is preserved by including it back in the column
name:
