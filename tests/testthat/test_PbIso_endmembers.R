...
if (!exists(".pb_iso_cols")) {
  .pb_iso_cols <- function() c("206Pb/204Pb", "207Pb/204Pb", "208Pb/204Pb")
}
...
test_that("pb_iso_endmembers throws error when required isotope columns are missing", {
  invalid_df <- data.frame(sample_id = "S1", `206Pb/204Pb` = 18.1, check.names = FALSE)
  class(invalid_df) <- c("ASTR", "data.frame")

  expect_error(
    pb_iso_endmembers(invalid_df),
