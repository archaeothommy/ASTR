...
$$
\delta^{y/x} = (\text{SSB Linear} - 1) \times 1000
$$
...
$$ R_{\text{corr,sample}} = \text{SSB Linear} \times R_{\text{true,std}} $$
...
$$
\text{SSB Linear} = \frac{R_{meas,sample}}{\text{mean}(R_{meas,std1}, R_{meas,std2})}
$$
...
$$
R_{corr,avg} = \text{mean}(\text{SSB Linear}_1, \text{SSB Linear}_2, \text{SSB Linear}_3)
$$
