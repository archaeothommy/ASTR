...
To mitigate the complexities introduced by metal mixing, [@eshel19] proposed a methodology utilizing "end-members" from a sample group.
...
This while defining endmembers as distinct groups is not always precise, it is preferable than analysing individual data-points separately, as it provides a more structured and meaningful interpretation of isotopic variability [@eshel19].
Then, assuming that the endmembers reflects a single ore, the provenance is suggested based on the highest probability of the cluster, considering all the results [@shnyr26].
...
This is achived with the use of the `stats::prcomp()` function, which is incorporated into the `pb_iso_endmembers()` function.
...
The ASTR object is with Lead isotpe rations 206Pb/204Pb, 207Pb/204Pb, 208Pb/204Pb
are apendted with lead `end_membr` column identivying each sample as wither belonging to an endmember group or the mixing group
