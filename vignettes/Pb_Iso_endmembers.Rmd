...
df <- ASTR::ArgentinaDatabase[ASTR::ArgentinaDatabase$`Mining area` == "Distrito El Aguilar", col_names]
end_members <- pb_iso_endmembers(df, names(df), tolerance = c(0.01, 0.01))
...
no_clamp <- pb_iso_endmembers(df, names(df))
print(no_clamp$group2)

# Example with clamping
clamp <- pb_iso_endmembers(df, names(df), clamp = c(Inf, 0.1))
print(clamp$group2)
