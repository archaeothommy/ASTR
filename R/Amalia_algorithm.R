#' Lead isotope provenance matching using the AMALIA algorithm
#'
...
#'   sample, the algorithm identifies analytically identical reference data
#'   by comparing lead isotope ratios within their combined analytical
...
#'   Analyses: Formulation and proof of concept at the Roman foundry of
#'   Fuente Spitz (Jaén, Spain). Journal of Archaeological Science: Reports
#'   51, 104192. <https://doi.org/10.1016/j.jasrep.2023.104192>
#'
...
#'   analytical errors and an ID column. Must contain the same column names
#'   as `df`.
#' @param isotope_204 Character vector of length 3 with column names of the
#'   204Pb-normalised isotope ratios. Must be present in both `df` and `ref`.
#'   Default to `c("206Pb/204Pb", "207Pb/204Pb", "208Pb/204Pb")`.
#' @param isotope_206 Character vector of length 3 with column names of the
#'   206Pb-normalised isotope ratios. Must be present in both `df` and `ref`.
#'   Only used when `triplet = "206"` or `triplet = "both"`. Default to
#'   `c("206Pb/204Pb", "207Pb/206Pb", "208Pb/206Pb")`.
#' @param error_204 Character vector of length 3 with column names of the
#'   analytical errors (2SD) for the 204Pb-normalised ratios. Must be present
#'   in both `df` and `ref`. Default to `c("206Pb/204Pb_err2SD",
#'   "207Pb/204Pb_err2SD", "208Pb/204Pb_err2SD")`.
#' @param error_206 Character vector of length 3 with column names of the
#'   analytical errors (2SD) for the 206Pb-normalised ratios. Must be present
...
#'   Default is `"ID"`.
#' @param id_ref String with the column name of the reference IDs in `ref`.
#'   Default is `"ID"`.
#' @param triplet Character string specifying which isotope ratio triplet to
#'   use for matching. One of `"204"` (206/204, 207/204, 208/204),
#'   `"206"` (206/204, 207/206, 208/206), or `"both"` (a match must pass
#'   both triplets simultaneously, following Rodríguez et al. 2023).
#'   Default is `"204"`.
#'
#' @return A list of three elements:
#'
#' * `matches`: `data.frame`. Every sample-reference pair that passed the
#'   AMALIA matching criteria, with their absolute differences per ratio.
#' * `n_matches`: `data.frame`. Summary table with the number of reference
#'   matches per sample.
#' * `unmatched`: `character`. IDs of samples with no matches in the
#'   reference data.
#'
#' @details For each sample-reference pair, AMALIA checks whether the
#'   absolute difference between their isotope ratios is smaller than or
#'   equal to the combined analytical uncertainty (sum of both 2SD errors)
#'   for all three ratios in the selected triplet simultaneously.
#'
...
amalia <- function(
    df,
    ref,
    isotope_204 = c("206Pb/204Pb", "207Pb/204Pb", "208Pb/204Pb"),
    isotope_206 = c("206Pb/204Pb", "207Pb/206Pb", "208Pb/206Pb"),
    error_204   = c("206Pb/204Pb_err2SD", "207Pb/204Pb_err2SD",
                    "208Pb/204Pb_err2SD"),
    error_206   = c("206Pb/204Pb_err2SD", "207Pb/206Pb_err2SD",
                    "208Pb/206Pb_err2SD"),
    id_sample   = "ID",
    id_ref      = "ID",
  ...
  checkmate::assert_character(isotope_204, len = 3)
  checkmate::assert_character(isotope_206, len = 3)
  ...
  if (triplet == "204Pb" || triplet == "both") {
    matches_204 <- run_amalia(isotope_204, error_204)
  }

  if (triplet == "206Pb" || triplet == "both") {
    matches_206 <- run_amalia(isotope_206, error_206)
  }
