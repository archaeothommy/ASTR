...
#### Matching approach using base R and regex ####
...
isotope_notation <- paste0("(", paste0(c("d", "e"), collapse = "|"),
                           ")(", isotopes_list, ")"
                           )
elemental_ratio <- paste0("\\(?(", ox_elem_list, ")([\\+-/](",
                          ox_elem_list, "))?\\)?/\\(?(",
                          ox_elem_list, ")([\\+-/](",
                          ox_elem_list, "))?\\)?"
                          )

# Underscore included to enforce concentrations having units,
# and to prevent partial matching in elemental ratios
...
test <- read_csv("./tests/testthat/input_format.csv")
...
matching$unit <- ifelse(matching$unit != -1, regmatches(matching$col, matching$unit), NA)
